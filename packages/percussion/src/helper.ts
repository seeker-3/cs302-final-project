...
export function audioToDrum(
  ...
  //minInterval then the number of tiles in between each beat needs to change for
...
//is then used to multiply the number of tiles after each current tile.
...
//randomly assigning amplitude values to each point.
//This will create white noise. This white noise vector
//will be used to make the rattling sound you get in some
...
//for the white noise aspect and the oscillator aspect. I do the same thing for the other
//two sounds (kick and HiHat) but with different settings.
const snareSound: SoundSettings = {
  noise: {
    frequency: 2500,
    gain: 0.05,
    time: 0.05,
...
//The playSnare function calls the playSound function with the appropriate
//settings. The other sounds will do the same for their functions
export const playSnare = () => playSound(snareSound)

// ___Kick __

const kickSound: SoundSettings = {
  noise: {
    frequency: 0,
    gain: 0,
    time: 0,
...
export const playKick = () => playSound(kickSound)

// ___Hi Hat ____

const hiHatSound: SoundSettings = {
  noise: {
    frequency: 10000,
    gain: 0.1,
    time: 0.1,
...
export const playHiHat = () => playSound(hiHatSound)
...
export async function playSound(sound: SoundSettings) {
  ...
  //of the noise exponentially over a given period of time
  const whiteNoiseSourceGain = audioContext.createGain()
  whiteNoiseSourceGain.gain.setValueAtTime(
    sound.noise.volume,
