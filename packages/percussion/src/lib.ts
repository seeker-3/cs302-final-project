...
function getCommonInterval(intervals) {
  ...
  const intervalRanks = [] //this will hold the number of occurencies for each catagory 
  ...
  while (intervals.length > 0) {
    ...
    for (; i < intervalCats.length; i++) {
      ...
      //and add one to its catagory rank. Then break to stop the search
    ...
    //and add it to the intervalCats to make a new catagory of interval. set its 
    //rank to 1
  ...
  //This for loop iterates over the interval ranks to find the most common occurence
...
// - minFreq is the smallest frequency that you are interseted in detecting.
//   It is used to avoid detecting the silences in between peaks in a notes frequeny
...
const ave = arr => arr.reduce((a,b) => Math.abs(a) + Math.abs(b), 0) / arr.length
...
function audioToMelody(audioData: Float32Array) {
  ...
  //minStep is the period of time inbetween the peaks of
  ...
  let notes = []//stores the index at which notes start 
  ...
  let intervals = []//This stores the intervals between the start of each note
  let interval //holds the current interval length

  //This for loop iterates of the notes vector to find the shortest interval between 
  //the start of two notes. The shortes interval, or quantLen, has to be found before recording all of 
  ...
  for (let i = 0; i < notes.length-1; i++) {
    ...
    //common interval significtly easier and more standardized
    interval = notes[i+1] - notes[i]
  ...
  //a note that is hummed twice quickly can be registered as two distict sounds rather than one 
  //continuous one. 
  let quant = getCommonInterval(intervals)
...
function audioToDrum(audioData: Float32Array) {
  ...
  const interval = Math.min.apply(null, intervals) / (audioContext.sampleRate / 1000)
  ...
  for (let b = 0; b < beats.length - 1; b++) {
    newDrum.push(1)
    const space = parseInt((intervals[b] / (minInterval * (audioContext.sampleRate / 1000))).toFixed())
