...
// AB - you typically don't want to pass Promises as arguments like that
// const convert = async (frequencies:  Promise<(number | null)[]>) => {
//   frequencies
//     .then(notes => {
//       let count = 0
...
const frequencyToNote = (input: any) => {
  const A4 = 440.0
  const A4_INDEX = 57
  ...
  // const result = notes[A4_INDEX + r_index]
  // if (side === PLUS) result += ' plus '
  // else result += ' minus '
  // result += cent_index + ' cents'
  // return result + (side === PLUS ? ' plus ' : ' minus ') + cent_index + ' cents'
  return `${notes[A4_INDEX + r_index]} ${
    side === PLUS ? 'plus' : 'minus'
  } ${cent_index} cents`
}
